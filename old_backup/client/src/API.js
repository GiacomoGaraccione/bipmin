...
async function getScores() {
  const response = await fetch("/api/logs/scores")
  if (!response.ok)
    throw new Error(response.statusText);
...
async function getAttempts() {
  const response = await fetch("/api/logs/attempts")
  if (!response.ok)
    throw new Error(response.statusText);
...
async function getTimestamps() {
  const response = await fetch("/api/logs/timestamps")
  if (!response.ok)
    throw new Error(response.statusText);
...
async function addSizes(username, exercise, mode, attempt, size) {
  let response = await fetch("/api/sizes", {
    method: "POST",
    headers: {
      "Content-Type": "application/json"
    },
    body: JSON.stringify({ username: username, exercise: exercise, mode: mode, attempt: attempt, size: size })
  })
  if (!response.ok) {
    try {
      const errDetail = await response.json();
...
async function addSyntax(username, exercise, mode, attempt, errors, errorCount) {
  let response = await fetch("/api/syntax", {
    method: "POST",
    headers: {
      "Content-Type": "application/json"
    },
    body: JSON.stringify({ username: username, exercise: exercise, mode: mode, attempt: attempt, errors: errors, errorCount: errorCount })
  })
  if (!response.ok) {
    try {
      const errDetail = await response.json();
...
async function addErrors(username, exercise, mode, attempt, errors) {
  let response = await fetch("/api/errors", {
    method: "POST",
    headers: {
      "Content-Type": "application/json"
    },
    body: JSON.stringify({ username: username, exercise: exercise, mode: mode, attempt: attempt, errors: errors })
  })
  if (!response.ok) {
    try {
      const errDetail = await response.json();
...
async function addOldRules(username, exercise, errors, errorCount, found, foundCount) {
  let response = await fetch("/api/old", {
    method: "POST",
    headers: {
      "Content-Type": "application/json"
    },
    body: JSON.stringify({ username: username, exercise: exercise, errors: errors, errorCount: errorCount, found: found, foundCount: foundCount })
  })
  if (!response.ok) {
    try {
      const errDetail = await response.json();
...
const API = {
  ...
  getScores, getRulesFound, getAttempts, getTimestamps, getParticipants, addSizes, addSyntax, addErrors, addOldRules
}
export default API;