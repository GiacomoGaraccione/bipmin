    ...
    <Container>
      {props.loggedIn ?
        <>
          ...
          {props.user.id <= 24 && <Navbar.Brand onClick={() => props.setMode("eval")}>Evaluator</Navbar.Brand>}
          <Navbar.Toggle />
          <Navbar.Collapse className="justify-content-end">
            <Navbar.Text className="me-4">
              Welcome {props.user.name}
            </Navbar.Text>
            <Button variant="outline-light" onClick={props.logout} href={"/login"} component={HashRouter}>Logout</Button>
          </Navbar.Collapse>
        </>
        :
